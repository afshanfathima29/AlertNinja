    ...
    def run_scan(self):
        ...
        try:
            ...
            if scan_type == "Ping Scan":
                ...
                nm.scan(hosts=target, arguments='-sV --script=vuln -v')
        ...
        for host in scanner.all_hosts():
            ...
            for port in sorted(ports):
                ...
                if 'script' in port_info:
                    self.results_text.insert(tk.END, "Vulnerabilities:\n")
                    for script_name, script_output in port_info['script'].items():
                        if isinstance(script_output, str):
                            # Handle single string output
                            self.results_text.insert(tk.END, 
                                f"  {script_name}: {script_output}\n")
                            ...
                            for item in script_output:
                                if isinstance(item, str):
                                    ...
                                    # Handle dictionary output (common in NSE scripts)
